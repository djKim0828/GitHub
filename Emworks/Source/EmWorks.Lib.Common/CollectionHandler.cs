...
namespace EmWorks.Lib.Common
{
    public class CollectionHandler<T> where T : new()
    {
        ...
        public CollectionHandler(string filePath = "", int storagePeriod = 180)
        {
        ...
        {// Todo: 파일 생성날자 기준으로
            try
            {
                ...
                foreach (string dir in dirs)
                {
                    ...
                    if (GetDayLength(dirInfo.CreationTime) > _storagePeriod)
                    {
        ...
        private int GetDayLength(DateTime dateValue)
        {
            TimeSpan sp = DateTime.Now - dateValue;
        ...
        private void StartDataWriter(string writeDir)
        {
            ...
            if (_writeFileWriteThread.IsAlive == false)
            {
        ...
        private bool WriteLine(DataWithSubPath dataWsPath)
        {
            ...
            else
            {
                try
                {
                    jsondata = ',' + _jsonConvertor.Serialize(dataWsPath._ModelData);
