...
namespace EmWorks.Lib.Common
{
    public class CollectionHandler<T> where T : new()
    {
        ...
        public bool LoadDictionary(string filePath, ref Dictionary<string, T> dictionaryData)
        {
            CreateDirectorySafely(filePath);
            try
            {
                string jsonString = File.ReadAllText(filePath);
                dictionaryData = _jsonConvertor.Deserialize<Dictionary<string, T>>(jsonString);
            }
            catch (Exception ex)
            {
                Logger.Exception(ex);
        ...
        /// <returns></returns>
        public bool SaveDictionary(string filePath, Dictionary<string, T> dictionaryData)
        {
            if (CreateDirectorySafely(filePath) == false)
            {
                return false;
            }

            string allText = _jsonConvertor.Serialize(dictionaryData);
