...
namespace EmWorks.App.OpticInspection
{
    ...
    public class AnalogCommand : EmWorks.View.AnalogCommandDialog
    {
        ...
        private void BtnEnter_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            {
                if (TempTag != null)
                    TempTag.Is = Convert.ToDouble(txbInputText.Text);
            }
            catch { }
            finally
            {
        ...
        private int InitControls()
        {
            try
            {
                if (TempTag != null)
                {
                    txbInputText.Text = TempTag.d.ToString();
                }
            }
            catch { }
        ...
        private double MakeValue(string value)
        {
            try
            {
                ...
                {
                    if (txbInputText.Text == "0" || (Convert.ToDouble(txbInputText.Text) == TempTag.d && _isFirst == true) || Convert.ToDouble(txbInputText.Text) == _result)
                    {
                        txbInputText.Text = "";
                        _isFirst = false;
                    }
