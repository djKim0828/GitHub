...
namespace EmWorks.App.OpticInspection
{
    ...
    /// </summary>
    public partial class InspectionCommander : StageBase
    {
        #region Fields

        public Dictionary<string, MotionPosition> _X3PostionList;
        public Dictionary<string, MotionPosition> _Z4PostionList;
        public Dictionary<string, MotionPosition> _Z5PostionList;

        private string _sequenceName = "InspectionSequence1";// Todo : 추후에 시퀀스를 Switch 할 수 있다.

        #endregion Fields

        #region Constructors

        public InspectionCommander(int stageId) : base(stageId)
        {
            Initialize();
        }
        ...
        public bool Move(int axisIndex, string position, int timeOut = 10000, bool isCheckSkip = false)
        {
            switch (axisIndex)
            {
                case Idx.MotionAxisNo.InspectorX3:
                    if (CommandCenter.Move(Global._PmacX3, _X3PostionList, position, timeOut) == false)
                    {
                        return false;
                    } // else
                ...
                case Idx.MotionAxisNo.CameraZ4:
                    if (CommandCenter.Move(Global._PmacZ4, _Z4PostionList, position, timeOut) == false)
                    {
                        return false;
                    } // else
                ...
                case Idx.MotionAxisNo.Sr5000Z5:
                    if (CommandCenter.Move(Global._PmacZ5, _Z5PostionList, position, timeOut) == false)
                    {
                        return false;
                    } // else
        ...
        private int InitInstance()
        {
            try
            {
                ...
                CommandCenter.LoadMotionPosition(Idx.MotionAxisNo.InspectorX3, ref _X3PostionList);
                CommandCenter.LoadMotionPosition(Idx.MotionAxisNo.CameraZ4, ref _Z4PostionList);
                CommandCenter.LoadMotionPosition(Idx.MotionAxisNo.Sr5000Z5, ref _Z5PostionList);
