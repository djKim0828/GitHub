    ...
    public partial class EmAjinMotion
    {
        ...
        private void RunSimCommand(Tag commandTag)
        {
            ...
            else if (commandTag.Identity.IoType == Tag.Idx.IoType.Output)
            {
                ...
                switch (protocols)
                {
                    case nameof(CAXM.AxmMoveStop):
                    case nameof(CAXM.AxmMoveEStop):
                    case nameof(CAXM.AxmMoveSStop):
                        // 정지
                        SimStopMotion();
                        break;

                    case nameof(CAXM.AxmMoveStartPos):
                        // 이동
        ...
        private void SimMoveStartPosProc(object tag)
        {
            try
            {
                ...
                // Busy Tag
                Tag xBusyTag = GetTag(commandTag.Identity.Unit, "AxmStatusReadInMotion");
                xBusyTag.Is = 1;

                // 명령 Tag
                Tag xCmdPosTag = GetTag(commandTag.Identity.Unit, "AxmStatusGetCmdPos");
                xCmdPosTag.Is = Convert.ToDouble(targetPos);

                // 정지 Tag
                Tag yEStopTag = GetTag(commandTag.Identity.Unit, "AxmMoveEStop");
                yEStopTag.Is = 0;

                // 실제 Tag
                Tag xActPosTag = GetTag(commandTag.Identity.Unit, "AxmStatusGetActPos");
