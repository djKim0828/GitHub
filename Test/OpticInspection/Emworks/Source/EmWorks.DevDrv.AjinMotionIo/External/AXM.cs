...
public class CAXM
{
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmInfoGetBoardFirstAxisNo(int lBoardNo, int lModulePos, ref int lpAxisNo);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetProfileMode(int lAxisNo, uint uProfileMode);
    ...
    public static uint AxmSignalReadLimitP(int nAxisNo, ref uint upStatus)
    {
        uint neg = 0;
        return AxmSignalReadLimit(nAxisNo, ref upStatus, ref neg);
    }
    public static uint AxmSignalReadLimitN(int nAxisNo, ref uint upStatus)
    {
        uint pos = 0;
        return AxmSignalReadLimit(nAxisNo, ref pos, ref upStatus);
    }
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalReadSoftLimit(int nAxisNo, ref uint upPositiveStatus, ref uint upNegativeStatus);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalServoOn(int nAxisNo, uint uOnOff);
    // 지정 축의 Servo-On 신호의 출력 상태를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalIsServoOn(int nAxisNo, ref uint upOnOff);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalSetFilterBandwidth(int nAxisNo, uint uSignal, double dBandwidthUsec);

    // Universal Output을 mSec 동안 On 유지하다가 Off 시킨다
    // nArraySize : 동작시킬 OutputBit배열의 수
    // nmSec : 0 ~ 30000
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalOutputOn(int nAxisNo, int nArraySize, uint[] upBitNo, uint umSec);

    // Universal Output을 mSec 동안 Off 유지하다가 On 시킨다
    // nArraySize : 동작시킬 OutputBit배열의 수
    // nmSec : 0 ~ 30000
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalOutputOff(int nAxisNo, int nArraySize, int[] npBitNo, int nmSec);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetPosType(int nAxisNo, uint uPosType, double dPositivePos, double dNegativePos);
    // 지정 축의 위치 정보 사용 방법에 대하여 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusGetPosType(int nAxisNo, ref uint upPosType, ref double dpPositivePos, ref double dpNegativePos);
    // 지정 축의 절대치 엔코더 원점 Offset 위치를 설정한다.[PCI-R1604-MLII 전용]
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetAbsOrgOffset(int nAxisNo, double dOrgOffsetPos);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetPosMatch(int nAxisNo, double dPos);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusRequestServoAlarm(int nAxisNo);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoAlarm(int nAxisNo, uint uReturnMode, ref uint upAlarmCode);
    // 지정한 에러코드에 해당하는 Alarm String을 받아오는 함수
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusGetServoAlarmString(int nAxisNo, uint uAlarmCode, int nAlarmStringSize, byte[] szAlarmString);

    // 지정한 축의 서보팩에 Alarm History를 읽어오도록 명령하는 함수
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusRequestServoAlarmHistory(int nAxisNo);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoAlarmHistory(int nAxisNo, uint uReturnMode, ref int npCount, ref uint upAlarmCode);
    // 지정한 축의 서보팩 Alarm History를 Clear한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusClearServoAlarmHistory(int nAxisNo);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmHomeSetFineAdjust(int nAxisNo, double dHomeDogLength, uint lLevelScanTime, uint uFineSearchUse, uint uHomeClrUse);
    // 설정되어있는 홈 관련 미세조정 파라메타들을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmHomeGetFineAdjust(int nAxisNo, ref double dpHomeDogLength, ref uint lpLevelScanTime, ref uint upFineSearchUse, ref uint upHomeClrUse);
    ...
    // Vel값이 양수이면 CW, 음수이면 CCW 방향으로 구동.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartMultiVel(int lArraySize, int[] lpAxesNo, double[] dVel, double[] dAccel, double[] dDecel);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartMultiVelEx(int lArraySize, int[] lpAxesNo, double[] dpVel, double[] dpAccel, double[] dpDecel, uint uSyncMode);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartLineVel(int lArraySize, int[] lpAxesNo, double[] dpDis, double dVel, double dAccel, double dDecel);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveSignalSearchAtDis(int nAxisNo, double dVel, double dAccel, double dDecel, int nDetectSignal, double dDis);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartMultiPos(int nArraySize, int[] nAxisNo, double[] dPos, double[] dVel, double[] dAccel, double[] dDecel);

    // 다축을 설정한 거리만큼 또는 위치까지 이동한다.
    // 지정 축들의 절대 좌표로 설정된 위치까지 설정된 속도와 가속율로 구동을 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveMultiPos(int nArraySize, int[] nAxisNo, double[] dPos, double[] dVel, double[] dAccel, double[] dDecel);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartTorque(int lAxisNo, double dTorque, double dVel, uint uAccFilterSel, uint uGainSel, uint uSpdLoopSel);

    // 지정 축의 토크 구동을 정지 한다.
    // AxmMoveStartTorque후 반드시 AxmMoveTorqueStop를 실행하여야 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveTorqueStop(int lAxisNo, uint uMethod);
    ...
    // 펄스 출력이 종료되는 시점에서 함수를 벗어난다
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartPosWithList(int lAxisNo, double dPosition, double[] dpVel, double[] dpAccel, double[] dpDecel, int lListNum);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartPosWithPosEvent(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel, int lEventAxisNo, double dComparePosition, uint uPositionSource);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStopEx(int lAxisNo, double dDecel);
    // 지정 축을 급 정지 한다.
    [DllImport("AXL.dll")]
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideVelAtMultiPos(int nAxisNo, double dPos, double dVel, double dAccel, double dDecel, int nArraySize, double[] dpOverridePos, double[] dpOverrideVel, int nTarget, uint uOverrideMode);
    ...
    //                : OVERRIDE_POS_END(1) 지정한 위치에서 지정한 속도가 되도록 미리 오버라이드 시작함
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideVelAtMultiPos2(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel, int lArraySize, double[] dpOverridePos, double[] dpOverrideVel, double[] dpOverrideAccelDecel, int lTarget, uint dwOverrideMode);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmGantrySetCompensationGain(int lMasterAxisNo, int lMasterGain, int lSlaveGain);
    // 모션 모듈은 두 축이 기구적으로 Link되어있는 겐트리 구동시스템 제어 중 동기 보상 기능을 설정을 확인한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmGantryGetCompensationGain(int lMasterAxisNo, ref int lMasterGain, ref int lSlaveGain);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmGantrySetErrorRange(int lMasterAxisNo, double dErrorRange, uint uUse);
    // Master 와 Slave 간의 오차 범위 설정값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmGantryGetErrorRange(int lMasterAxisNo, ref double dpErrorRange, ref uint upUse);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmGantryReadErrorRangeStatus(int lMasterAxisNo, ref uint dwpStatus);
    // Master 와 Slave 간의 오차값을 반환 한다.
    // Flag Latch 모드 일때 Latch 된 값을 사용자가 읽어 간 후 다음 Error가 발생 되기 전까지 Error가 발생 했을 때의 값을 유지 하게 됩니다.
    [DllImport("AXL.dll")]
    public static extern uint AxmGantryReadErrorRangeComparePos(int lMasterAxisNo, ref double dpComparePos);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmLineMove(int lCoord, double[] dPos, double dVel, double dAccel, double dDecel);

    // 2축 단위 직선 보간 한다.(Software 방식)
    // 시작점과 종료점을 지정하여 다축 직선 보간 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    [DllImport("AXL.dll")]
    public static extern uint AxmLineMoveEx2(int lCoord, double[] dpEndPos, double dVel, double dAccel, double dDecel);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmCircleCenterMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double[] dEndPos, double dVel, double dAccel, double dDecel, uint uCWDir);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmCirclePointMove(int lCoord, int[] lAxisNo, double[] dMidPos, double[] dEndPos, double dVel, double dAccel, double dDecel, int lArcCircle);
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmCircleRadiusMove(int lCoord, int[] lAxisNo, double dRadius, double[] dEndPos, double dVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance);
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmCircleAngleMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double dAngle, double dVel, double dAccel, double dDecel, uint uCWDir);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmContiSetAxisMap(int lCoord, uint lSize, int[] lpRealAxesNo);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmContiStart(int lCoord, uint uProfileset, int lAngle);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmContiSetConnectionRadius(int lCoord, double dRadius);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmTriggerOnlyAbs(int nAxisNo, int nTrigNum, double[] dTrigPos);
    // 트리거 설정을 리셋한다.
    [DllImport("AXL.dll")]
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmSplineWrite(int lCoord, int lPosSize, double[] dPosX, double[] dPosY, double dVel, double dAccel, double dDecel, double dPosZ, int lPointFactor);

    //======== PCI-R1604-MLII/SIIIH, PCIe-Rxx04-SIIIH 전용 함수 ================================================================================== 
    // 위치 보정 테이블 기능에 필요한 내용을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationSet(int nAxisNo, int nNumEntry, double dStartPos, ref double dpPosition, ref double dpCorrection, uint uRollOver);
    // 위치 보정 테이블 기능 설정 내용을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationGet(int nAxisNo, ref int npNumEntry, ref double dpStartPos, ref double dpPosition, ref double dpCorrection, ref uint upRollOver);

    // 위치 보정 테이블 기능의 사용유부를 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationEnable(int nAxisNo, uint uEnable);
    // 위치 보정 테이블 기능의 사용유무에 대한 설정 상태를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationIsEnable(int nAxisNo, ref uint upEnable);
    // 현재 지령 위치에서의 보정값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationGetCorrection(int lAxisNo, ref double dpCorrection);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationSetBacklash(int lAxisNo, int lBacklashDir, double dBacklash);
    // Backlash에 관련된 설정 내용을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationGetBacklash(int lAxisNo, ref int lpBacklashDir, ref double dpBacklash);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationEnableBacklash(int lAxisNo, uint dwEnable);
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationIsEnableBacklash(int lAxisNo, ref uint dwpEnable);
    ...
    //   - [4303] -> Backlash 보정기능이 설정되어있지않을 때
    [DllImport("AXL.dll")]
    public static extern uint AxmCompensationSetLocating(int lAxisNo, double dVel, double dAccel, double dDecel, double dWaitTime);

    // ECAM 기능에 필요한 내용을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamSet(int nAxisNo, int nMasterAxisNo, int nNumEntry, double dMasterStartPos, ref double dpMasterPos, ref double dpSlavePos);
    // ECAM 기능에 필요한 내용을 CMD/ACT Source와 함께 설정한다. (PCIe-Rxx04-SIIIH 전용 함수)
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamSetWithSource(int lAxisNo, int lMasterAxis, int lNumEntry, double dMasterStartPos, ref double dpMasterPos, ref double dpSlavePos, uint dwSource);
    // ECAM 기능 설정 내용을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamGet(int nAxisNo, ref int npMasterAxisNo, ref int npNumEntry, ref double dpMasterStartPos, ref double dpMasterPos, ref double dpSlavePos);
    // ECAM 기능 설정 내용을 CMD/ACT Source와 함께 반환한다. (PCIe-Rxx04-SIIIH 전용 함수)
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamGetWithSource(int lAxisNo, ref int lpMasterAxis, ref int lpNumEntry, ref double dpMasterStartPos, ref double dpMasterPos, ref double dpSlavePos, ref uint dwpSource);

    // ECAM 기능의 사용 유무를 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamEnableBySlave(int nAxisNo, uint uEnable);
    // ECAM 기능의 사용 유무를 지정한 Master 축에 연결된 모든 Slave 축에 대하여 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamEnableByMaster(int nAxisNo, uint uEnable);
    // ECAM 기능의 사용 유무에 대한 설정 상태를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEcamIsSlaveEnable(int nAxisNo, ref uint upEnable);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetServoMonitor(int nAxisNo, uint uSelMon, double dActionValue, uint uAction);
    // 지정 축의 예외 처리 기능에 대한 설정 상태를 반환한다.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusGetServoMonitor(int nAxisNo, uint uSelMon, ref double dpActionValue, ref uint upAction);
    // 지정 축의 예외 처리 기능에 대한 사용 유무를 설정한다.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetServoMonitorEnable(int nAxisNo, uint uEnable);
    // 지정 축의 예외 처리 기능에 대한 사용 유무를 반환한다.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusGetServoMonitorEnable(int nAxisNo, ref uint upEnable);

    // 지정 축의 예외 처리 기능 실행 결과 플래그 값을 반환한다. 함수 실행 후 자동 초기화.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoMonitorFlag(int nAxisNo, uint uSelMon, ref uint upMonitorFlag, ref double dpMonitorValue);
    // 지정 축의 예외 처리 기능을 위한 감시 정보를 반환한다.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoMonitorValue(int nAxisNo, uint uSelMon, ref double dpMonitorValue);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusSetReadServoLoadRatio(int lAxisNo, uint dwSelMon);
    // 지정 축의 부하율을 반환한다.(MLII : Sigma-5, SIIIH : MR_J4_xxB 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoLoadRatio(int lAxisNo, ref double dpMonitorValue);

    //======== PCI-R1604-RTEX 전용 함수==================================================================================
    // RTEX A4Nx 관련 Scale Coefficient를 설정한다.(RTEX, A4Nx 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetScaleCoeff(int nAxisNo, int lScaleCoeff);
    // RTEX A4Nx 관련 Scale Coefficient 를 확인한다.(RTEX, A4Nx 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetScaleCoeff(int nAxisNo, ref int lpScaleCoeff);
    // 특정 Input 신호의 Edge를 검출하여 즉정지 또는 감속정지하는 함수.
    // lDetect Signal : edge 검출할 입력 신호 선택.
    // lDetectSignal  : PosEndLimit(0), NegEndLimit(1), HomeSensor(4), EncodZPhase(5), UniInput02(6), UniInput03(7)
    // Signal Edge    : 선택한 입력 신호의 edge 방향 선택 (rising or falling edge).
    ...
    //          PCI-Nx04를 사용할 경우 lDetectSignal이 PosEndLimit , NegEndLimit(0,1) 을 찾을경우 신호의레벨 Active 상태를 검출하게된다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveSignalSearchEx(int nAxisNo, double dVel, double dAccel, int nDetectSignal, int nSignalEdge, int nSignalMethod);
    //-------------------------------------------------------------------------------------------------------------------
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveToAbsPos(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel);
    // 지정 축의 현재 구동 속도를 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadVelEx(int lAxisNo, ref double dpVel);
    //-------------------------------------------------------------------------------------------------------------------
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetElectricGearRatio(int lAxisNo, int lNumerator, int lDenominator);
    // 지정 축의 전자 기어비 설정을 확인한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetElectricGearRatio(int lAxisNo, ref int lpNumerator, ref int lpDenominator);
    ...
    // 최대 토크의 0.1% 단위로 제어 함.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetTorqueLimit(int lAxisNo, double dbPluseDirTorqueLimit, double dbMinusDirTorqueLimit);
    ...
    // 최대 토크의 0.1% 단위로 제어 함.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetTorqueLimit(int lAxisNo, ref double dbpPluseDirTorqueLimit, ref double dbpMinusDirTorqueLimit);
    ...
    // dReserved      : 사용하지 않음.
    [DllImport("AXL.dll")]
    public static extern uint AxmOverridePosSetFunction(int lAxisNo, uint dwUsage, int lDecelPosRatio, double dReserved);
    // 지정 축의 AxmOverridePos에 대한 특수 기능 사용 유무를 확인 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmOverridePosGetFunction(int lAxisNo, ref uint dwpUsage, ref int lpDecelPosRatio, ref double dpReserved);
    ...
    // dwpImpulseCount   : 2 ~ 5
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvVSTSetParameter(int lCoordinate, uint dwISTSize, double[] dbpFrequency, double[] dbpDampingRatio, ref uint dwpImpulseCount);
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvVSTGetParameter(int lCoordinate, ref uint dwpISTSize, double[] dbpFrequency, double[] dbpDampingRatio, ref uint dwpImpulseCount);
    // lCoordnate        : 입력 성형 코디 번호를 입력한다.
    // dwISTEnable       : 입력 성형 사용 유무를 결정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvVSTSetEnabele(int lCoordinate, uint dwISTEnable);
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvVSTGetEnabele(int lCoordinate, ref uint dwISTEnable);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvLineMove(int lCoordinate, double[] dPosition, double dMaxVelocity, double dStartVel, double dStopVel, double dMaxAccel, double dMaxDecel);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvOvrLineMove(int lCoordinate, double[] dPosition, double dMaxVelocity, double dStartVel, double dStopVel, double dMaxAccel, double dMaxDecel, int lOverrideMode);
    // 2축 원호보간 한다.
    // 시작점, 종료점과 중심점을 지정하여 원호 보간 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvCircleCenterMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir);
    // 중간점, 종료점을 지정하여 원호 보간 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    // AxmAdvContiBeginNode, AxmAdvContiEndNode와 같이사용시 지정된 좌표계에 중간점, 종료점을 지정하여 구동하는 원호 보간 Queue에 저장함수가된다.
    // 프로파일 원호 연속 보간 구동을 위해 내부 Queue에 저장하여 AxmAdvContiStart함수를 사용해서 시작한다.
    // lAxisNo = 두축 배열 , dMidPos = 중간점 X,Y 배열 , dEndPos = 종료점 X,Y 배열, lArcCircle = 아크(0), 원(1)
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvCirclePointMove(int lCoord, int[] lAxisNo, double[] dMidPos, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, int lArcCircle);
    // 시작점, 회전각도와 반지름을 지정하여 원호 보간 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvCircleAngleMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double dAngle, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir);
    // 시작점, 종료점과 반지름을 지정하여 원호 보간 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvCircleRadiusMove(int lCoord, int[] lAxisNo, double dRadius, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvOvrCircleRadiusMove(int lCoord, int[] lAxisNo, double dRadius, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, int lOverrideMode);
    ...
    // uCWDir DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향    	
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvHelixCenterMove(int lCoord, double dCenterXPos, double dCenterYPos, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir);
    // 지정된 좌표계에 시작점, 종료점과 반지름을 지정하여 헬리컬 보간 구동하는 함수이다. 
    // AxmAdvContiBeginNode, AxmAdvContiEndNode와 같이사용시 지정된 좌표계에 중간점, 종료점을 지정하여 헬리컬연속 보간 구동하는 함수이다. 
    // 원호 연속 보간 구동을 위해 내부 Queue에 저장하는 함수이다. AxmAdvContiStart함수를 사용해서 시작한다. (연속보간 함수와 같이 이용한다.)
    // dMidPos = 중간점 X,Y  , dEndPos = 종료점 X,Y 
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvHelixPointMove(int lCoord, double dMidXPos, double dMidYPos, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel);
    // 지정된 좌표계에 시작점, 회전각도와 반지름을 지정하여 헬리컬 보간 구동하는 함수이다
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvHelixAngleMove(int lCoord, double dCenterXPos, double dCenterYPos, double dAngle, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir);
    // 지정된 좌표계에 시작점, 종료점과 반지름을 지정하여 헬리컬 보간 구동하는 함수이다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향   
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvHelixRadiusMove(int lCoord, double dRadius, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvOvrHelixRadiusMove(int lCoord, double dRadius, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, int lOverrideMode);
    ...
    // 직선 프로파일 연속 보간 구동을 위해 내부 Queue에 저장하여 AxmAdvContiStart함수를 사용해서 시작한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptLineMove(int lCoordinate, double[] dPosition, double dMaxVelocity, double dStartVel, double dStopVel, double dMaxAccel, double dMaxDecel, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // 내부적으로 오버라이드 큐에 있는 예약 보간들이 연속보간 큐로 저장되고, 직선 오버라이드 구동과 이후의 예약된 연속보간이 순차적으로 실행된다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptOvrLineMove(int lCoordinate, double[] dPosition, double dMaxVelocity, double dStartVel, double dStopVel, double dMaxAccel, double dMaxDecel, int lOverrideMode, uint dwScript, int lScirptAxisNo, double dScriptPos);
    // 2축 원호보간을 예약 구동한다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptCircleCenterMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint dwScript, int lScirptAxisNo, double dScriptPos);
    // 중간점, 종료점을 지정하여 원호 보간을 예약 구동하는 함수이다. 구동 시작 후 함수를 벗어난다.
    // AxmAdvContiBeginNode, AxmAdvContiEndNode와 같이사용시 지정된 좌표계에 중간점, 종료점을 지정하여 구동하는 원호 보간 Queue에 저장함수가된다.
    // 프로파일 원호 연속 보간 구동을 위해 내부 Queue에 저장하여 AxmAdvContiStart함수를 사용해서 시작한다.
    // lAxisNo = 두축 배열 , dMidPos = 중간점 X,Y 배열 , dEndPos = 종료점 X,Y 배열, lArcCircle = 아크(0), 원(1)
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptCirclePointMove(int lCoord, int[] lAxisNo, double[] dMidPos, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, int lArcCircle, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptCircleAngleMove(int lCoord, int[] lAxisNo, double[] dCenterPos, double dAngle, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptCircleRadiusMove(int lCoord, int[] lAxisNo, double dRadius, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // 내부적으로 오버라이드 큐에 있는 예약 보간들이 연속보간 큐로 저장되고, 원호 오버라이드 구동과 이후의 예약된 연속보간이 순차적으로 실행된다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptOvrCircleRadiusMove(int lCoord, int[] lAxisNo, double dRadius, double[] dEndPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, int lOverrideMode, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // uCWDir DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향	
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptHelixCenterMove(int lCoord, double dCenterXPos, double dCenterYPos, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // dMidPos = 중간점 X,Y  , dEndPos = 종료점 X,Y
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptHelixPointMove(int lCoord, double dMidXPos, double dMidYPos, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptHelixAngleMove(int lCoord, double dCenterXPos, double dCenterYPos, double dAngle, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint dwScript, int lScirptAxisNo, double dScriptPos);
    // 지정된 좌표계에 시작점, 종료점과 반지름을 지정하여 헬리컬 보간을 예약 구동하는 함수이다.
    ...
    // uCWDir   DIR_CCW(0): 반시계방향, DIR_CW(1) 시계방향  
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptHelixRadiusMove(int lCoord, double dRadius, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, uint dwScript, int lScirptAxisNo, double dScriptPos);
    ...
    // 내부적으로 오버라이드 큐에 있는 예약 보간들이 연속보간 큐로 저장되고, 헬리컬 오버라이드 구동과 이후의 예약된 연속보간이 순차적으로 실행된다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvScriptOvrHelixRadiusMove(int lCoord, double dRadius, double dEndXPos, double dEndYPos, double dZPos, double dVel, double dStartVel, double dStopVel, double dAccel, double dDecel, uint uCWDir, uint uShortDistance, int lOverrideMode, uint dwScript, int lScirptAxisNo, double dScriptPos);

    //====연속 보간 함수 ================================================================================================
    // 지정된 좌표계에 연속 보간 구동 중 현재 구동중인 연속 보간 인덱스 번호를 확인하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiGetNodeNum(int lCoordinate, ref int lpNodeNum);
    // 지정된 좌표계에 설정한 연속 보간 구동 총 인덱스 갯수를 확인하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiGetTotalNodeNum(int lCoordinate, ref int lpNodeNum);
    // 지정된 좌표계에 보간 구동을 위한 내부 Queue에 저장되어 있는 보간 구동 개수를 확인하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiReadIndex(int lCoordinate, ref int lpQueueIndex);
    // 지정된 좌표계에 보간 구동을 위한 내부 Queue가 비어 있는지 확인하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiReadFree(int lCoordinate, ref uint upQueueFree);
    // 지정된 좌표계에 연속 보간 구동을 위해 저장된 내부 Queue를 모두 삭제하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiWriteClear(int lCoordinate);
    // 지정된 좌표계에 연속 보간 오버라이드 구동을 위해 예약된 오버라이드용 큐를 모두 삭제하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvOvrContiWriteClear(int lCoordinate);
    // 연속 보간 시작 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiStart(int lCoord, uint dwProfileset, int lAngle);
    // 연속 보간 정지 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiStop(int lCoordinate, double dDecel);
    //지정된 좌표계에 연속보간 축 맵핑을 설정한다.
    //(축맵핑 번호는 0 부터 시작))
    // 주의점:  축맵핑할때는 반드시 실제 축번호가 작은 숫자부터 큰숫자를 넣는다.
    //          가상축 맵핑 함수를 사용하였을 때 가상축번호를 실제 축번호가 작은 값 부터 lpAxesNo의 낮은 인텍스에 입력하여야 한다.
    ...
    //          같은 축을 다른 Coordinate에 중복 맵핑하지 말아야 한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiSetAxisMap(int lCoord, int lSize, int[] lpAxesNo);
    //지정된 좌표계에 연속보간 축 맵핑을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiGetAxisMap(int lCoord, ref int lpSize, ref int lpAxesNo);
    // 지정된 좌표계에 연속보간 축 절대/상대 모드를 설정한다.
    // (주의점 : 반드시 축맵핑 하고 사용가능)
    // 지정 축의 이동 거리 계산 모드를 설정한다.
    //uAbsRelMode : POS_ABS_MODE '0' - 절대 좌표계
    //              POS_REL_MODE '1' - 상대 좌표계
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiSetAbsRelMode(int lCoord, uint uAbsRelMode);
    // 지정된 좌표계에 연속보간 축 절대/상대 모드를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiGetAbsRelMode(int lCoord, ref uint uAbsRelMode);
    // 지정된 좌표계에 연속 보간 구동 중인지 확인하는 함수이다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiIsMotion(int lCoordinate, ref uint upInMotion);
    // 지정된 좌표계에 연속보간에서 수행할 작업들의 등록을 시작한다. 이함수를 호출한후,
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiBeginNode(int lCoord);
    // 지정된 좌표계에서 연속보간을 수행할 작업들의 등록을 종료한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvContiEndNode(int lCoord);

    // 지정한 다축을 설정한 감속도로 동기 감속 정지한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveMultiStop(int lArraySize, int[] lpAxesNo, double[] dMaxDecel);
    // 지정한 다축을 동기 급 정지한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveMultiEStop(int lArraySize, int[] lpAxesNo);
    // 지정한 다축을 동기 감속 정지한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveMultiSStop(int lArraySize, int[] lpAxesNo);

    // 지정한 축의 실제 구동 속도를 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadActVel(int lAxisNo, ref double dpVel);          //2010.10.11
    // 서보 타입 슬레이브 기기의 SVCMD_STAT 커맨드 값을 읽는다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoCmdStat(int lAxisNo, ref uint upStatus);
    // 서보 타입 슬레이브 기기의 SVCMD_CTRL 커맨드 값을 읽는다.
    [DllImport("AXL.dll")]
    public static extern uint AxmStatusReadServoCmdCtrl(int lAxisNo, ref uint upStatus);

    // 겐트리 구동시 마스터 축과 슬레이브 축 간의 위치 차에 대한 설정된 오차 한계값을 반환한다.    
    [DllImport("AXL.dll")]
    public static extern uint AxmGantryGetMstToSlvOverDist(int lAxisNo, ref double dpPosition);
    // 겐트리 구동시 마스터 축과 슬레이브 축 간의 위치 차에 대한 오차 한계값을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmGantrySetMstToSlvOverDist(int lAxisNo, double dPosition);

    // 지정 축의 알람 신호의 코드 상태를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalReadServoAlarmCode(int lAxisNo, ref ushort upCodeStatus);

    // 서보 타입 슬레이브 기기의 좌표계 설정을 실시한다. (MLIII 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoCoordinatesSet(int lAxisNo, uint dwPosData, uint dwPos_sel, uint dwRefe);
    // 서보 타입 슬레이브 기기의 브레이크 작동 신호를 출력한다. (MLIII 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoBreakOn(int lAxisNo);
    // 서보 타입 슬레이브 기기의 브레이크 작동 신호를 해제한다. (MLIII 전용)
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoBreakOff(int lAxisNo);
    // 서보 타입 슬레이브 기기의 
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoConfig(int lAxisNo, uint dwCfMode);
    // 서보 타입 슬레이브 기기의 센서 종보 초기화를 요구한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSensOn(int lAxisNo);
    // 서보 타입 슬레이브 기기의 센서전원 OFF를 요구한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSensOff(int lAxisNo);
    // 서보 타입 슬레이브 기기의 SMON 커맨드를 실행한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSmon(int lAxisNo);
    // 서보 타입 슬레이브 기기의 모니터 정보나 입출력 신호의 상태를 읽는다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetSmon(int lAxisNo, ref uint pbParam);
    // 서보 타입 슬레이브 기기에 서보 ON을 요구한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSvOn(int lAxisNo);
    // 서보 타입 슬레이브 기기에 서보 OFF를 요구한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSvOff(int lAxisNo);
    // 서보 타입 슬레이브 기기가 지정된 보간 위치로 보간이동을 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoInterpolate(int lAxisNo, uint dwTPOS, uint dwVFF, uint dwTFF, uint dwTLIM);
    // 서보 타입 슬레이브 기기가 지정한 위치로 위치이송을 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoPosing(int lAxisNo, uint dwTPOS, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM);
    // 서보 타입 슬레이브 기기가 지정된 이동속도로 정속이송을 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoFeed(int lAxisNo, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM);
    // 서보 타입 슬레이브 기기가 이송중 외부 위치결정 신호의 입력에 의해 위치이송을 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoExFeed(int lAxisNo, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM, uint dwExSig1, uint dwExSig2);
    // 서보 타입 슬레이브 기기가 외부 위치결정 신호 입력에 의해 위치이송을 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoExPosing(int lAxisNo, uint dwTPOS, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM, uint dwExSig1, uint dwExSig2);
    // 서보 타입 슬레이브 기기가 원점 복귀를 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoZret(int lAxisNo, uint dwSPD, uint dwACCR, uint dwDECR, uint dwTLIM, uint dwExSig1, uint dwExSig2, uint bHomeDir, uint bHomeType);
    // 서보 타입 슬레이브 기기가 속도제어를 실시한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoVelctrl(int lAxisNo, uint dwTFF, uint dwVREF, uint dwACCR, uint dwDECR, uint dwTLIM);
    // 서보 타입 슬레이브 기기가 토크제어를 실시한다.    
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoTrqctrl(int lAxisNo, uint dwVLIM, int lTQREF);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetParameter(int lAxisNo, uint wNo, uint bSize, uint bMode, ref uint pbParam);
    // 서보 타입 슬레이브 기기의 서보팩 특정 파라미터 값을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetParameter(int lAxisNo, uint wNo, uint bSize, uint bMode, ref uint pbParam);

    // M3 서보팩에 Command Execution을 실행한다
    [DllImport("AXL.dll")]
    public static extern uint AxmServoCmdExecution(int lAxisNo, uint dwCommand, uint dwSize, ref uint pdwExcData);
    // 서보 타입 슬레이브 기기의 설정된 토크 제한 값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetTorqLimit(int lAxisNo, ref uint dwpTorqLimit);
    // 서보 타입 슬레이브 기기에 토크 제한 값을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetTorqLimit(int lAxisNo, uint dwTorqLimit);

    // 서보 타입 슬레이브 기기에 설정된 SVCMD_IO 커맨드 값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetSendSvCmdIOOutput(int lAxisNo, ref uint dwData);
    // 서보 타입 슬레이브 기기에 SVCMD_IO 커맨드 값을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetSendSvCmdIOOutput(int lAxisNo, uint dwData);

    // 서보 타입 슬레이브 기기에 설정된 SVCMD_CTRL 커맨드 값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetSvCmdCtrl(int lAxisNo, ref uint dwData);
    // 서보 타입 슬레이브 기기에 SVCMD_CTRL 커맨드 값을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetSvCmdCtrl(int lAxisNo, uint dwData);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetMonSel(int lAxisNo, uint dwMon0, uint dwMon1, uint dwMon2);
    // 서보 축 추가 모니터링 채널별 설정 값을 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetMonSel(int lAxisNo, ref uint upMon0, ref uint upMon1, ref uint upMon2);
    // 서보 축 추가 모니터링 채널별 설정 값을 기준으로 현재 상태를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoReadMonData(int lAxisNo, uint dwMonSel, ref uint dwpMonData);
    // 제어할 토크 축 설정
    [DllImport("AXL.dll")]
    public static extern uint AxmAdvTorqueContiSetAxisMap(int lCoord, int lSize, int[] lpAxesNo, uint dwTLIM, uint dwConMode);
    // 2014.04.28
    // 토크 프로파일 설정 파라미터
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoSetTorqProfile(int lCoord, int lAxisNo, int TorqueSign, uint dwVLIM, uint dwProfileMode, uint dwStdTorq, uint dwStopTorq);
    // 토크 프로파일 확인 파라미터
    [DllImport("AXL.dll")]
    public static extern uint AxmM3ServoGetTorqProfile(int lCoord, int lAxisNo, ref int lpTorqueSign, ref uint updwVLIM, ref uint upProfileMode, ref uint upStdTorq, ref uint upStopTorq);
    //-------------------------------------------------------------------------------------------------------------------
    //======== SMP 전용 함수 =======================================================================================
    // Inposition 신호의 Range를 설정한다. (dInposRange > 0)
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalSetInposRange(int lAxisNo, double dInposRange);
    // Inposition 신호의 Range를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmSignalGetInposRange(int lAxisNo, ref double dpInposRange);

    // 오버라이드할때 위치속성(절대/상대)을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetOverridePosMode(int lAxisNo, uint dwAbsRelMode);
    // 오버라이드할때 위치속성(절대/상대)을 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetOverridePosMode(int lAxisNo, ref uint dwpAbsRelMode);
    // LineMove 오버라이드할때 위치속성(절대/상대)을 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetOverrideLinePosMode(int lCoordNo, uint dwAbsRelMode);
    // LineMove 오버라이드할때 위치속성(절대/상대)을 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetOverrideLinePosMode(int lCoordNo, ref uint dwpAbsRelMode);

    // AxmMoveStartPos와 동일하며 종료속도(EndVel)가 추가되었다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveStartPosEx(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel, double dEndVel);
    // AxmMovePos와 동일하며 종료속도(EndVel)가 추가되었다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMovePosEx(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel, double dEndVel);

    // Coordinate Motion을 경로상에서 감속정지(dDecel) 시킨다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveCoordStop(int lCoordNo, double dDecel);
    // Coordinate Motion을 급정지 시킨다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveCoordEStop(int lCoordNo);
    // Coordinate Motion을 경로상에서 감속정지 시킨다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMoveCoordSStop(int lCoordNo);

    // AxmLineMove모션의 위치를 오버라이드 시킨다.
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideLinePos(int lCoordNo, ref double dpOverridePos);
    // AxmLineMove모션의 속도를 오버라이드 시킨다. 각축들의 속도비율은 dpDistance로 결정된다.
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideLineVel(int lCoordNo, double dOverrideVel, ref double dpDistance);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideLineAccelVelDecel(int lCoordNo, double dOverrideVelocity, double dMaxAccel, double dMaxDecel, ref double dpDistance);
    // AxmOverrideVelAtPos에 추가적으로 AccelDecel을 오버라이드 시킨다.
    [DllImport("AXL.dll")]
    public static extern uint AxmOverrideAccelVelDecelAtPos(int lAxisNo, double dPos, double dVel, double dAccel, double dDecel, double dOverridePos, double dOverrideVel, double dOverrideAccel, double dOverrideDecel, int lTarget);
    ...
    [DllImport("AXL.dll")]
    public static extern uint AxmEGearSet(int lMasterAxisNo, int lSize, ref int lpSlaveAxisNo, ref double dpGearRatio);
    // Electronic Gearing정보를 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEGearGet(int lMasterAxisNo, ref int lpSize, ref int lpSlaveAxisNo, ref double dpGearRatio);
    // Electronic Gearing을 해제한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEGearReset(int lMasterAxisNo);
    // Electronic Gearing을 활성/비활성한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEGearEnable(int lMasterAxisNo, uint dwEnable);
    // Electronic Gearing을 활성/비활성상태를 읽어온다.
    [DllImport("AXL.dll")]
    public static extern uint AxmEGearIsEnable(int lMasterAxisNo, ref uint dwpEnable);

    // 주의사항: 입력한 종료속도가 '0'미만이면 '0'으로, 'AxmMotSetMaxVel'로 설정한 최대속도를 초과하면 'MaxVel'로 재설정된다. 
    // 지정 축에 종료속도를 설정한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotSetEndVel(int lAxisNo, double dEndVelocity);
    // 지정 축의 종료속도를 반환한다.
    [DllImport("AXL.dll")]
    public static extern uint AxmMotGetEndVel(int lAxisNo, ref double dpEndVelocity);

    [DllImport("AXL.dll")]
    public static extern uint AxmFilletMove(int lCoord, double[] dPos, double[] dFVector, double[] dSVector, double dVel, double dAccel, double dDecel, double dRadius);
}
